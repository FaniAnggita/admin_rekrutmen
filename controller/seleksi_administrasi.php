...
for ($i = 0; $i < count($selectedIdsArray); $i++) {
    echo "ID: " . $selectedIdsArray[$i] . "\n";
}
...
foreach ($selectedIdsArray as $id_pelamar) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (mysqli_query($conn, $update_sql)) {
            ...
            echo "Error updating data for ID $id_pelamar: " . mysqli_error($conn) . "\n";
        }
    } else {
        // Insert a new record
        ...
        if (mysqli_query($conn, $insert_sql)) {
            ...
            echo "Error inserting data for ID $id_pelamar: " . mysqli_error($conn) . "\n";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($result) > 0) {
        // Update the existing record
        $update_sql = "UPDATE seleksi_administrasi SET tanggal_seleksi = '$tanggal_seleksi', nilai_cv = '$nilai_cv',
nilai_kualifikasi = '$nilai_kualifikasi', nilai_pengalaman = '$nilai_pengalaman', hasil_seleksi_adm = '$hasil',
keterangan_adm = '$keterangan' WHERE id_pelamar = '$id_pelamar'";
        ...
        // Insert a new record
        $insert_sql = "INSERT INTO seleksi_administrasi (id_pelamar, tanggal_seleksi, nilai_cv, nilai_kualifikasi,
nilai_pengalaman, hasil_seleksi_adm, keterangan_adm) VALUES ('$id_pelamar', '$tanggal_seleksi', '$nilai_cv',
'$nilai_kualifikasi', '$nilai_pengalaman', '$hasil', '$keterangan')";
